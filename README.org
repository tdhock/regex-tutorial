...
- =stringr::str_match= and =stringi::stri_match= implement extracting
  capture groups, but [[https://github.com/hadley/stringr/pull/16][the stringi package developer does not seem to
  think that named capture is supported]], even though [[http://userguide.icu-project.org/strings/regexp][named capture
  does appear in the ICU regexp docs]].
