...
for which there no delimiter between groups (as in CSV/TSV), but still
some structure.
...
so I wrote the [[file:str_match.R][str_match_perl and str_match_all_perl wrapper functions]]
that return character matrices, with column names as defined in the
named groups of the regular expression.
...
the =S.str.findall= method which does not support named capture.
...
|---------------------------+--------------------------+-------------------------+-----------------------------|
| regexpr(P, S, perl=TRUE)  | str_match_perl(S, P)     | S.str.extract(P, flags) | one match per subject       |
| gregexpr(P, S, perl=TRUE) | str_match_all_perl(S, P) | S.str.findall(P, flags) | several matches per subject |
