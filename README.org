...
Python 1.5 introduced named capture groups and the =(?P<name>foo)=
...
function =str_match_re2= that uses the [[https://github.com/google/re2/wiki/CplusplusAPI][RE2::FullMatch]] C++ function to
obtain a match matrix like the output of =str_match_named=. Run a
benchmark on matching the regular expression =a?a?a?aaa= against the
string =aaa= (for up to 30 repetitions). Does your result agree with
the predictions in the complexity column of the table below?
